...
def run(plan, args):
    ...
    for idx, chain in enumerate(chain_configs):
        ...
        teleporter_registry_address = contract_deployer.deploy_teleporter_registry(plan, chain_info["RPCEndpointBaseURL"])
    ...
    for idx, chain in enumerate(chain_configs):
        ...
        for idx, dest_chain_name in enumerate(bridge_config["destinations"]):
            contract_deployer.deploy_token_remote(plan, l1_info[dest_chain_name]["RPCEndpointBaseURL"], "TOK", l1_info[dest_chain_name]["TeleporterRegistryAddress"], l1_info[source_chain_name]["BlockchainIdHex"], token_home_address)
