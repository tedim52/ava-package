...
DEPLOYER_PK="56289e99c94b6912bfc12adc093c9b51124f0dc54ac7a766b2bc5ccf558d8027"
...
def deploy_teleporter_registry(plan, chain_rpc_url, chain_name):
    ...
    deploy_result =plan.run_sh(
        ...
        run="/home/deploy_registry.sh --version {0} --rpc-url {1} --private-key {2} > /home/registry_address.txt".format(TELEPORTER_REGISTRY_VERSION, chain_rpc_url, DEPLOYER_PK),
        image=FOUNDRY_IMAGE,
        files = {
...
def deploy_teleporter_messenger(plan, chain_rpc_url, chain_name):
    ...
    deploy_result =plan.run_sh(
        ...
        run="/home/deploy_teleporter.sh --version {0} --rpc-url {1} --private-key {2}".format(TELEPORTER_REGISTRY_VERSION, chain_rpc_url, DEPLOYER_PK),
        image=FOUNDRY_IMAGE,
        files = {
            "/home": deploy_teleporter_script,
        },
    )
    teleporter_messenger = deploy_result.output
...
def deploy_token_home(plan, chain_rpc_url, token_name, teleporter_registry_address, erc20_token_address): 
    plan.print("deploy token home cmd: forge create --rpc-url {0} --private-key {1} lib/avalanche-interchain-token-transfer/contracts/src/TokenHome/ERC20TokenHome.sol:ERC20TokenHome --constructor-args {2} {3} {4} 18".format(chain_rpc_url, DEPLOYER_PK, teleporter_registry_address, FUNDED_ADDRESS, erc20_token_address))
    result = plan.run_sh(
        description="Deploying TokenHome contract to L1",
        name="token-home-deployer",
        run="forge create --rpc-url {0} --private-key {1} lib/avalanche-interchain-token-transfer/contracts/src/TokenHome/ERC20TokenHome.sol:ERC20TokenHome --constructor-args {2} {3} {4} 18 | grep -oP '(?<=Deployed to: )0x[0-9a-fA-F]+$' | tr -d '\n'".format(chain_rpc_url, DEPLOYER_PK, teleporter_registry_address, FUNDED_ADDRESS, erc20_token_address),
        image=FOUNDRY_IMAGE,
    )
    ...
    plan.run_sh(
        ...
        run="cast send --rpc-url {0} --private-key {1} {2} \"approve(address, uint256)\" {3} 2000000000000000000000".format(chain_rpc_url, DEPLOYER_PK, erc20_token_address, token_home_contract_address),
        image=FOUNDRY_IMAGE,
    )
...
def deploy_token_remote(plan, chain_rpc_url, token_name, dest_teleporter_registry_address, source_blockchain_id_hex, source_token_home_address): 
    ...
    plan.run_sh(
        ...
        run="cast send --rpc-url {0} --private-key {1} {2} \"registerWithHome((address, uint256))\" \"(0x0000000000000000000000000000000000000000, 0)\"".format(chain_rpc_url, DEPLOYER_PK, token_remote_address),
        image=FOUNDRY_IMAGE,
    )
